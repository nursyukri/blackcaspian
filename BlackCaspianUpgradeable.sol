...
import "./BlackCaspian.sol";


contract BlackCaspianUpgradeable is CustodianUpgradeable  {
    ...
    // CONSTRUCTOR
    function BlackCaspianUpgradeable(address _custodian) CustodianUpgradeable(_custodian) public {
        BlackCaspian = BlackCaspian(0x0);
    }

    // MODIFIERS
    modifier onlyImpl {
        require(msg.sender == address(BlackCaspian));
        _;
    }
    ...
    /** @notice  Confirms a pending change of the active implementation
      ...
      * pending change, the `BlackCaspian BlackCaspian` member will be updated
      * with the requested address.
      *
      * @param  _lockId  The identifier of a pending change request.
      */
    function confirmImplChange(bytes32 _lockId) public onlyCustodian {
        BlackCaspian = getImplChangeReq(_lockId);

        delete implChangeReqs[_lockId];

        emit ImplChangeConfirmed(_lockId, address(BlackCaspian));
    }

    // PRIVATE FUNCTIONS
    function getImplChangeReq(bytes32 _lockId) private view returns (BlackCaspian _proposedNew) {
        ...
        return BlackCaspian(changeRequest.proposedNew);
    }
